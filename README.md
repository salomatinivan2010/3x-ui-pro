...
За основу взят 3x-ui-pro от mozaroc в котором исправлено получение подписок, могло не работать в некоторых случаях.
Исправлено не работавшее из-за ошибке в скрипте подключение xhttp
добавлено открытие портов ufw использованных в скрипте.
Панель 3x-ui от MHSanaei заменена на lucx-ui, Добавлена резервная копия в /root/lucx-backup-ДАТА-ВРЕМЯ,
Сохраняются:
UUID, пароли, subId, лимиты и сроки действия;
включённое/выключенное состояние и счётчики трафика;
привязки к REALITY, WS, XHTTP и Trojan gRPC;
пути подписок — при сохранении домена ссылки остаются прежними.
Добавляется inbound Telegram WEB proxy с автоматически созданным секретом.
Подключение работает через основной домен и Nginx на 443; внутренний relay слушает localhost.
В конце выводится ссылка https://t.me/webproxy?server=…&secret=….

Автоматическая установка панели [[3x-ui](https://github.com/MHSanaei/3x-ui](https://github.com/AlexeyLCP/lucx-ui)) с nginx, SSL, Clash-подпиской и диагностикой сети.
...
```bash
wget -qO x-ui-latest.sh https://raw.githubusercontent.com/salomatinivan2010/3x-ui-pro/refs/heads/main/x-ui-latest.sh
```
